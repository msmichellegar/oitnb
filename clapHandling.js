...
var addClap = function (data) {
  ...
  out += data.username !== "anonymous" ? '<p>via' + data.username + 'at ' + timeParser(new Date(Number(data.time))) + '</p>' : '';
  out += data.userId === userId ? '<button class="delButtons">x</button></br></br></div>' : '';
  return out;
